...
class DataReader:
    ...
    def _di_roi(self, mirrors):
        ...
        return np.round([DI.center - s - 3*_sigma, DI.center + 3*_sigma]).astype(int)
    ...
    def _cropping(self, data, mirrors=None):
        ...
        if self.measure == 'spade':
            cropped = self._spade_reader(temp).T
        if self.measure == 'di':
    ...
    def _reshaping(self, data, length, mirrors=None):
        temp = data[:length, :]
        shape = 2 if self.measure == 'spade' else -np.subtract(*self._di_roi(mirrors))
        temp = temp.reshape(200, -1, shape)[:, :50, :]
        return temp
    ...
    def _processing(self, avi, length, mirrors=None):
        ...
        reshaped = self._reshaping(cropped, length, mirrors)
        return reshaped, noise
